...
def test_healthy_card_says_how_much_ran():
    ...
    assert v["workloads_completed"] == 2 and v["summary"] == "2 workload(s) completed"
...
def test_thermal_limit_is_a_watch_with_the_temperature_in_it():
    v = pantheon.assess_gpu([_row("memory_read", 1900.0, max_temp=95, throttle_reason="Thermal")], 0, "H")
    assert v["verdict"] == "WATCH"
    assert "thermal: memory_read thermally throttled, GPU at 95 C" in v["reasons"][0]
    assert v["summary"] == "thermally throttled on 1"
    assert v["findings"]["throttled"] == [{"test": "memory_read", "temp_c": 95.0}]
...
def test_hot_but_unthrottled_card_and_hot_memory_are_watches():
    ...
    assert v["verdict"] == "WATCH" and "hot: mma_virus GPU reached 91 C" in v["reasons"][0]
    v = pantheon.assess_gpu([_row("memory_read", 900.0, max_mem_temp=96)], 0, "X")
    assert v["verdict"] == "WATCH" and "memory_read memory reached 96 C" in v["reasons"][0]
...
def test_findings_are_grouped_by_kind_and_ordered_worst_first():
    ...
    v = pantheon.assess_gpu(rows, 0, "H")
    assert v["verdict"] == "WATCH"
    kinds = [r.split(":")[0] for r in v["reasons"]]
    assert kinds == ["thermal", "hot", "correctable errors on 2 workload(s)"]
    assert "memory_hammer GPU reached 92 C, memory_write GPU reached 90 C" in v["reasons"][1]
    assert "pcie.bad_tlp +3, pcie.lcrc +1" in v["reasons"][2]           # summed across workloads
    assert v["summary"] == "thermally throttled on 1; hot on 2; correctable errors on 2"
    text = pantheon.format_verdict(v)
    assert text.splitlines()[0].endswith("(thermally throttled on 1; hot on 2; correctable errors on 2)")
    assert "  ! thermal: memory_read thermally throttled, GPU at 95 C" in text
    assert "note: PCIe link recovery events on 1 workload(s)" in text


def test_format_verdict_carries_the_word_the_summary_and_the_reasons():
    v = pantheon.assess_gpu([_row("memory_read", 868.4)], 0, "NVIDIA GeForce RTX 3080 Ti")
    text = pantheon.format_verdict(v)
