...
// --- ENTROPY GENERATOR ---
// Generates uncompressible noise to blind the memory compressor
...
// Fills the VRAM with uncompressible high-entropy noise before the read test begins.
// Without this, the DCC engine will compress the zero-filled memory and bypass the physical bus.
__global__ void init_memory_read_kernel(uint4* data, size_t n, int inject_error, int init_pattern) {
    ...
    for (size_t i = idx; i < n; i += stride) {
        ...
        // Handle standard memory patterns
        if (init_pattern == 0) {
            final_val = 0x00000000;
        } else if (init_pattern == 1) {
            final_val = 0xFFFFFFFF;
        } else {
            // Entropy defeats DCC compression; the base selects the toggle
            // behaviour (2 = crosstalk, 3 = rail-to-rail).
            uint32_t entropy = fast_hash((uint32_t)i);
            final_val = pantheon_pattern_value(i, init_pattern, entropy);
        }
        
        // --- DYNAMIC FAULT INJECTION ---
        if (inject_error && i == 1337) {
            // Intentionally corrupt the payload in memory
            final_val ^= 0xBADBEEF;
        }

        data[i] = make_uint4(final_val, final_val, final_val, final_val);
    }
}

// --- READ KERNEL (Coalesced Max Bandwidth) ---
__global__ void memory_read_kernel(uint4* data, size_t n, uint4* sink, int loops) {
    size_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    size_t stride = blockDim.x * gridDim.x;

    // Local accumulator to force the compiler to issue the load instructions
    ...
    for (int l = 0; l < loops; ++l) {
        ...
        for (size_t i = idx; i < n; i += stride * 16) {
            ...
            for (int u = 0; u < 16; ++u) {
                ...
                if (offset < n) {
                    uint4 val = data[offset];
                    
                    // XOR the loaded data into the accumulator
                    acc.x ^= val.x;
                    acc.y ^= val.y;
                    acc.z ^= val.z;
                    acc.w ^= val.w;
    ...
    // Sink the accumulator to global memory to prevent aggressive dead-code elimination.
    // This condition will practically never be met, making it virtually zero-overhead.
    if (acc.x == 0xDEADBEEF) {
        sink[idx] = acc;
    }
}
...
// --- VERIFICATION KERNEL ---
__global__ void verify_memory_read_kernel(uint4* data, size_t n, unsigned int* err_count, int init_pattern, PantheonFaultLog fault_log) {
    ...
    for (size_t offset = blockIdx.x * blockDim.x + threadIdx.x; offset < n; offset += stride) {
        ...
        // Recalculate the exact expected base for this specific memory address
        ...
        if (actual.x != expected_val || actual.y != expected_val || actual.z != expected_val || actual.w != expected_val) {
            ...
            if (*err_count < 5) {
                printf("[SDC FAULT][Memory_READ] Retention/Read Error! Index: %llu | Exp: 0x%08x | Act: 0x%08x | XOR: 0x%08x\n",
                       (unsigned long long)offset, expected_val, actual.x, xor_val);
            }

            // Record the full location for the fault map. Console output is
            // capped, so this is what makes a complete map possible.
...
int main(int argc, char* argv[]) {
    ...
    int init_pattern = 2;      // 0=Zeroes, 1=Ones, 2=Crosstalk+entropy, 3=Rail-to-rail+entropy
    std::string fault_map_path; // --fault_map <file>: write every failing address
    ...
    // Allocate the sink array to prevent compiler optimization
    size_t sink_size = num_blocks * block_size * sizeof(uint4);
    uint4* d_sink;
    CHECK(hipMalloc(&d_sink, sink_size));

    // --- PRINT ARGUMENTS ---
    std::cout << "[PANTHEON] GPU " << gpu_id << ": Memory READ (Entropy Coalesced) | " 
    ...
    // --- PHASE 1: INITIALIZE VRAM ---
    std::cout << "[PANTHEON] Initializing VRAM pattern..." << std::endl;
    int init_grid = init_launch_grid_size(prop, num_elements, block_size);
    LAUNCH_KERNEL(init_memory_read_kernel, init_grid, block_size, d_data, num_elements, inject_error, init_pattern);
    ...
    if (warmup_iters > 0) {
        ...
        for(int i = 0; i < warmup_iters; i++) {
            LAUNCH_KERNEL(memory_read_kernel, num_blocks, block_size, d_data, num_elements, d_sink, kernel_loops);
        }
        CHECK(hipDeviceSynchronize());
        // No memory reset required here because the read kernel doesn't mutate the source array.
    }
    ...
    if (verify_mode) {
        std::cout << "[PANTHEON] Running Memory Read Verification Pass..." << std::endl;
        ...
        LAUNCH_KERNEL(verify_memory_read_kernel, verify_grid, 256, d_data, num_elements, d_err_count, init_pattern, fault_log);
        CHECK(hipDeviceSynchronize());
        ...
        if (!fault_map_path.empty()) {
            pantheon_fault_log_write(fault_log, fault_map_path.c_str(),
                                     "memory_read", gpu_id, init_pattern);
