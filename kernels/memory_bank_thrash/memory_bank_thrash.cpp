...
// --- BANK THRASHING VIRUS ---
// Forces massive row-buffer misses by striding across 2MB memory boundaries.
__global__ void memory_bank_thrash_kernel(uint4* data, size_t n, unsigned int* sink, int loops, int inject_error) {
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    
    // 2MB stride in bytes = 2,097,152 bytes = 131,072 uint4s. 
    // This is a common threshold to hit the same channel/bank but force a page miss.
    size_t evil_stride = 131072; 
    
    unsigned int acc = 0;
    
    // Stagger starting positions so threads don't coalesce perfectly
    size_t curr_idx = (tid * 16) % evil_stride; 
    ...
    for (int i = 0; i < loops; ++i) {
        ...
        // Jump by exactly 2MB. 
        curr_idx = (curr_idx + evil_stride);
        if (curr_idx >= n) curr_idx = curr_idx % n; // Wrap around safely

        // --- DYNAMIC FAULT INJECTION ---
        if (inject_error && tid == 1337 && i == 500) {
            // Intentionally corrupt the accumulator
            acc += 0xBADBEEF; 
        }
    }
...
// --- VERIFICATION KERNEL ---
__global__ void verify_memory_kernel(unsigned int* sink, size_t n, unsigned int expected_val, unsigned int* err_count) {
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid < n) {
        ...
        if (actual_val != expected_val) {
            ...
            printf("[SDC FAULT][Memory_BANK_THRASH] Read Error! TID: %llu | Exp: 0x%08x | Act: 0x%08x | XOR: 0x%08x\n",
...
int main(int argc, char* argv[]) {
    ...
    int kernel_loops = 50000;  // Maps to bank stride reads
    int warmup_iters = 5;      
    ...
    // --- PRINT ARGUMENTS ---
    std::cout << "[PANTHEON] GPU " << gpu_id << ": Memory BANK THRASHER (Row Misses) | " 
    ...
    if (warmup_iters > 0) {
        ...
        for(int i = 0; i < warmup_iters; i++) {
            LAUNCH_KERNEL(memory_bank_thrash_kernel, num_blocks, block_size, d_data, num_elements, d_sink, kernel_loops, inject_error);
        }
    ...
    while (true) {
        ...
        // 16 bytes per read
        ops_performed += (size_t)num_blocks * block_size * kernel_loops * 16;
    ...
    // --- 6. VERIFICATION PASS ---
    if (verify_mode) {
        std::cout << "[PANTHEON] Running Memory Row Buffer Verification Pass..." << std::endl;
        
        unsigned int* d_err_count;
        CHECK(hipMalloc(&d_err_count, sizeof(unsigned int)));
        CHECK(hipMemset(d_err_count, 0, sizeof(unsigned int)));
        
        // Determine expected accumulator value. If memory was pre-filled with 0xFF,
        // each read will pull 0xFFFFFFFF. We accumulate this 'kernel_loops' times per launch.
        ...
        LAUNCH_KERNEL(verify_memory_kernel, verify_blocks, 256, d_sink, total_threads, expected_val, d_err_count);
        CHECK(hipDeviceSynchronize());
